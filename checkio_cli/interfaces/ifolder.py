...
def use_active(parser):
    ...
    def run(options):
        if options.mission != '-':
            config.set_value('active_mission', options.mission)
        if options.interpreter:
            config.set_value('active_interpreter', options.interpreter)

    parser.set_defaults(func=run)


def use_get(parser):  # TODO: can be used by simple checkio-cli get
    parser.add_argument('source')
    parser.add_argument('url')
    parser.add_argument('slug')

    def run(options):
        MISSION_GETTERS[options.source](options.url, options.slug)
        recompile_mission(options.slug)
        rebuild_mission(options.slug)
        init_home_file(options.slug, config.ACTIVE_INTERPRETER)
    parser.set_defaults(func=run)
...
def use(parser):
    ...
    use_get(subparsers.add_parser('get', help='Download and prepare mission'))
